...
instance MonadUnliftIO Handler where
  ...
  askUnliftIO = pure $ UnliftIO (\(Handler (ExceptT ioEither)) -> ioEither >>= either (const (error "throw away servant err")) pure)
...
afterLogin backend cookies = do
  let header = ("Location", "https://servant-and-login-with.com:8443/index.html")
  case cookies >>= lookup "jwt" of
    Nothing -> error "no jwt cookie"
    Just jwt -> do
      pPrint jwt
      let textJWT = decodeUtf8 jwt
      let maybeVerifiedJWT = decodeAndVerifySignature (secret "foobarbaz") textJWT
            ...
            Just profileJSON ->
              case profileJSON ^? key "username" . _String of
                ...
                Just username -> do
                  ...
                  throwError $ err302 { errHeaders = [header] }
